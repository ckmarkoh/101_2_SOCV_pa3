...
class V3NtkModule
{
   public : 
      ...
      V3NtkModule(const V3NetVec& i, const V3NetVec& o) : _inputs(i), _outputs(o) { _ref = 0; }
      ~V3NtkModule();
   ...
   private : 
      size_t            _ref;
      const V3NetVec    _inputs;
      const V3NetVec    _outputs;
};
...
class V3Ntk
{
   public : 
      ...
      void computeFanout();
      void computeLevel();
      inline const uint32_t getLevelSize() const;
      inline const uint32_t getLevel(const V3NetId&) const;
      inline const uint32_t getInputNetSize(const V3NetId&) const;
      inline const uint32_t getOutputNetSize(const V3NetId&) const;
      inline const V3NetId& getInputNetId(const V3NetId&, const uint32_t&) const;
      inline const V3NetId& getOutputNetId(const V3NetId&, const uint32_t&) const;
      inline const bool isFanoutComputed() const { return _outputNet.size() == _inputData.size(); }
      inline const bool isLevelComputed() const { return _levelData.size() == _inputData.size(); }
   ...
   private : 
      ...
      void dfsComputeFanoutList(const V3NetId&);
      void dfsComputeLevel(const V3NetId&);
   protected : 
      ...
      V3UI32Vec      _levelData;    // Level from PI / PIO / DFF (Forward)
      V3NetTable     _outputNet;    // Fanout Table for V3NetId  (V3NetId Only)
      ...
      // V3 Network Statistics
      uint32_t       _levelSize;    // Number of Levels
...
class V3BvNtk : public V3Ntk
{
   public : 
      ...
      const V3BitVecX* const getInputConstValue(const V3NetId&) const;
...
inline const uint32_t V3Ntk::getLevelSize() const {
   assert (isLevelComputed()); return _levelSize; }
inline const uint32_t V3Ntk::getLevel(const V3NetId& id) const {
   assert (validNetId(id)); assert (isLevelComputed()); return _levelData[id.id]; }
inline const uint32_t V3Ntk::getInputNetSize(const V3NetId& id) const {
   assert (validNetId(id)); return _inputData[id.id].size(); }
inline const uint32_t V3Ntk::getOutputNetSize(const V3NetId& id) const {
   assert (validNetId(id)); assert (isFanoutComputed()); return _outputNet[id.id].size(); }
...
inline const V3NetId& V3Ntk::getOutputNetId(const V3NetId& id, const uint32_t& i) const {
   assert (i < getOutputNetSize(id)); return _outputNet[id.id][i]; }
