...
void
V3AlgSimulate::reset() { resetSimulator(); }
...
void
V3AlgSimulate::clearSource(const V3NetId& id) {}
...
V3AlgSimulate::resetSimulator() {
   _cycle = 0; _init[0] = _init[1] = V3NtkUD; _orderMap.clear(); _hasEvent.clear(); _eventList.clear();
}
...
V3AlgSimulate::initializeEventList() {
   ...
   if (!_eventList.buckets()) {
      ...
      if (!ntk->isFanoutComputed()) ntk->computeFanout();
      if (!ntk->isLevelComputed()) ntk->computeLevel();
      assert (ntk->getLevelSize()); _eventList.init(ntk->getLevelSize());
   }
   else _eventList.clear();
   assert (_eventList.buckets() == ntk->getLevelSize()); assert (!_eventList.size());
}
...
void
V3AlgAigSimulate::reset() {
   // Initialize Simulation Order
   V3Ntk* const ntk = _handler->getNtk(); assert (ntk); resetSimulator();
   _init[0] = ntk->getConstSize() + ntk->getInputSize() + ntk->getInoutSize() + ntk->getLatchSize();
   _init[1] = dfsNtkForSimulationOrder(_handler->getNtk(), _orderMap); assert (_init[1] >= _init[0]);
   ...
   _dffValue = V3AigSimDataVec(ntk->getLatchSize()); _traceData.clear(); _cycle = 0;
...
void
V3AlgAigSimulate::clearSource(const V3NetId& id) {
   assert (id.id < _simValue.size()); _simValue[id.id].clear();
}
...
V3AlgAigSimulate::updateNextStateEvent() {
   ...
   for (uint32_t i = 0; i < ntk->getLatchSize(); ++i) {
      ...
      _eventList.add(ntk->getLevel(ntk->getLatch(i)), ntk->getLatch(i));
   }
...
V3AlgAigSimulate::setSourceEvent(const V3NetId& id, const V3BitVecX& value) {
   ...
   _eventList.add(_handler->getNtk()->getLevel(id), id);
   // Simulate According To Event List
   simulateEventList();
}
   ...
   if (V3_PI == type)
      for (uint32_t i = 0; i < ntk->getInputSize(); ++i) 
         if (!random) {
            ...
            _eventList.add(ntk->getLevel(ntk->getInput(i)), ntk->getInput(i));
         }
         else {
      ...
      for (uint32_t i = 0; i < ntk->getInoutSize(); ++i) 
         if (!random) {
            ...
            _eventList.add(ntk->getLevel(ntk->getInout(i)), ntk->getInout(i));
         }
         else {
...
V3AlgAigSimulate::simulateEventList() {
   ...
   while (_eventList.size()) {
      _eventList.pop(level, id); assert (level == ntk->getLevel(id));
      assert (_hasEvent[id.id]); _hasEvent[id.id] = false;
      for (uint32_t i = 0; i < ntk->getOutputNetSize(id); ++i) {
         id1 = ntk->getOutputNetId(id, i); assert (ntk->getLevel(id1)); value = _simValue[id1.id];
         assert (AIG_NODE == ntk->getGateType(id1)); assert (level < ntk->getLevel(id1));
         ...
         _hasEvent[id1.id] = true; _eventList.add(ntk->getLevel(id1), id1);
      }
   }
}
...
void
V3AlgBvSimulate::reset() {
   // Initialize Simulation Order
   V3BvNtk* const ntk = dynamic_cast<V3BvNtk*>(_handler->getNtk()); assert (ntk); resetSimulator();
   _init[0] = ntk->getConstSize() + ntk->getInputSize() + ntk->getInoutSize() + ntk->getLatchSize();
   _init[1] = dfsNtkForSimulationOrder(_handler->getNtk(), _orderMap); assert (_init[1] >= _init[0]);
   // Initialize Simulation Data and Reset Value
   _simValue = V3BvSimDataVec(ntk->getNetSize()); assert (_orderMap.size() <= _simValue.size());
   for (uint32_t i = 0; i < ntk->getConstSize(); ++i) {
      assert (_orderMap[i] == ntk->getConst(i));
      _simValue[ntk->getConst(i).id] = *(ntk->getInputConstValue(ntk->getConst(i)));
   }
...
void
V3AlgBvSimulate::clearSource(const V3NetId& id) {
   assert (id.id < _simValue.size()); _simValue[id.id].clear();
}
...
V3AlgBvSimulate::simulateEventList() {
   ...
   while (_eventList.size()) {
      ...
      for (uint32_t i = 0; i < ntk->getOutputNetSize(id); ++i) {
         id1 = ntk->getOutputNetId(id, i); assert (ntk->getLevel(id1)); value = _simValue[id1.id];
         assert (level < ntk->getLevel(id1)); simulateGate(id1);
         if (_hasEvent[id1.id] || value == _simValue[id1.id]) continue;
         _hasEvent[id1.id] = true; _eventList.add(ntk->getLevel(id1), id1);
      }
   }
