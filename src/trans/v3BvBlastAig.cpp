...
V3BvBlastAig::getInputName(const uint32_t& index) const {
   assert (_ntk); assert (index < _ntk->getInputSize()); assert (_handler);
   ...
   assert (i < ntk->getInputSize()); assert (pIndex < ntk->getNetWidth(ntk->getInput(i)));
   string name = _handler->getInputName(i); if (!name.size()) return name;
   if (1 == ntk->getNetWidth(ntk->getInput(i))) return name;
   else return name + (V3NameBitPrefix + v3Int2Str(pIndex) + V3NameBitSuffix);
}

const string
V3BvBlastAig::getOutputName(const uint32_t& index) const {
   assert (_ntk); assert (index < _ntk->getOutputSize()); assert (_handler);
   const V3Ntk* const ntk = _handler->getNtk(); assert (ntk);
   uint32_t i = 0, pIndex = index;
   for (; i < ntk->getOutputSize(); ++i)
      if (pIndex < ntk->getNetWidth(ntk->getOutput(i))) break;
      else pIndex -= ntk->getNetWidth(ntk->getOutput(i));
   assert (i < ntk->getOutputSize()); assert (pIndex < ntk->getNetWidth(ntk->getOutput(i)));
   string name = _handler->getOutputName(i); if (!name.size()) return name;
   if (1 == ntk->getNetWidth(ntk->getOutput(i))) return name;
   else return name + (V3NameBitPrefix + v3Int2Str(pIndex) + V3NameBitSuffix);
}

const string
V3BvBlastAig::getInoutName(const uint32_t& index) const {
   assert (_ntk); assert (index < _ntk->getInoutSize()); assert (_handler);
   const V3Ntk* const ntk = _handler->getNtk(); assert (ntk);
   uint32_t i = 0, pIndex = index;
   for (; i < ntk->getInoutSize(); ++i)
      if (pIndex < ntk->getNetWidth(ntk->getInout(i))) break;
      else pIndex -= ntk->getNetWidth(ntk->getInout(i));
   assert (i < ntk->getInoutSize()); assert (pIndex < ntk->getNetWidth(ntk->getInout(i)));
   string name = _handler->getInoutName(i); if (!name.size()) return name;
   if (1 == ntk->getNetWidth(ntk->getInout(i))) return name;
   else return name + (V3NameBitPrefix + v3Int2Str(pIndex) + V3NameBitSuffix);
}
...
V3BvBlastAig::getNetName(V3NetId& id, string& name) const {
   ...
   if (name.size() && 1 < _handler->getNtk()->getNetWidth(getParentNetId(netId)))
      name += (V3NameBitPrefix + v3Int2Str(getParentIndex(netId)) + V3NameBitSuffix);
}
...
V3BvBlastAig::getCurrentNetId(const V3NetId& id, const uint32_t& index) const {
   ...
   assert (index < _handler->getNtk()->getNetWidth(id));
...
V3BvBlastAig::performNtkTransformation() {
   ...
   for (i = 0; i < moduleList.size(); ++i) {
      ...
      for (uint32_t j = 0; j < pOutputs.size(); ++j) {
         ...
         for (uint32_t k = 0; k < ntk->getNetWidth(pOutputs[j]); ++k) 
            cOutputs.push_back(V3NetId::makeNetId(_p2cMap[pOutputs[j].id][k].id, pOutputs[j].cp));
      }
      ...
      for (uint32_t j = 0; j < pInputs.size(); ++j) {
         ...
         for (uint32_t k = 0; k < ntk->getNetWidth(pInputs[j]); ++k) 
            cInputs.push_back(V3NetId::makeNetId(_p2cMap[pInputs[j].id][k].id, pInputs[j].cp));
      }
...
V3BvBlastAig::bitBlast_ADD(const V3BvNtk* const ntk, const V3NetId& id) {
   ...
   const uint32_t width = ntk->getNetWidth(id); V3NetId carry;
...
V3BvBlastAig::bitBlast_MULT(const V3BvNtk* const ntk, const V3NetId& id) {
   ...
   V3NetId carry; V3NetVec prevResult(width);
   ...
   assert (carry.id == (_p2cMap[id.id][0].id + ((width * (width + 1)) >> 1) - 1));
...
V3BvBlastAig::bitBlast_DIV(const V3BvNtk* const ntk, const V3NetId& id) {
   assert (ntk); assert (BV_DIV == ntk->getGateType(id)); assert (!_p2cMap[id.id].size());
   const V3NetId id1 = ntk->getInputNetId(id, 0); assert (id1.id < ntk->getNetSize());
   const V3NetId id2 = ntk->getInputNetId(id, 1); assert (id2.id < ntk->getNetSize());
   assert (ntk->getNetWidth(id1) == _p2cMap[id1.id].size()); assert (ntk->getNetWidth(id2) == _p2cMap[id2.id].size());
   assert (ntk->getNetWidth(id1) == ntk->getNetWidth(id)); _p2cMap[id.id].reserve(ntk->getNetWidth(id));
   ...
   Msg(MSG_WAR) << "DIV CELL has not been implemented for V3BvBlastAig !!!" << endl;
}

void
V3BvBlastAig::bitBlast_MODULO(const V3BvNtk* const ntk, const V3NetId& id) {
   assert (ntk); assert (BV_MODULO == ntk->getGateType(id)); assert (!_p2cMap[id.id].size());
   const V3NetId id1 = ntk->getInputNetId(id, 0); assert (id1.id < ntk->getNetSize());
   const V3NetId id2 = ntk->getInputNetId(id, 1); assert (id2.id < ntk->getNetSize());
   assert (ntk->getNetWidth(id1) == _p2cMap[id1.id].size()); assert (ntk->getNetWidth(id2) == _p2cMap[id2.id].size());
   assert (ntk->getNetWidth(id1) == ntk->getNetWidth(id)); _p2cMap[id.id].reserve(ntk->getNetWidth(id));
   ...
   Msg(MSG_WAR) << "MODULO CELL has not been implemented for V3BvBlastAig !!!" << endl;
}
...
V3BvBlastAig::bitBlast_SHL(const V3BvNtk* const ntk, const V3NetId& id) {
   ...
   if (BV_CONST == ntk->getGateType(id2) && (sizeof(int) << 3) > ntk->getNetWidth(id2)) {
      const V3BitVecX* const value = ntk->getInputConstValue(id2); assert (value);
   ...
   uint32_t shift_width = (int)ceil(log2(width));
...
V3BvBlastAig::bitBlast_SLICE(const V3BvNtk* const ntk, const V3NetId& id) {
   ...
   cout << "Prev = " << _handler->getNetNameOrFormedWithId(id1) << ", expr = " << _handler->getNetExpression(id1) << endl;
   cout << "Cur = " << _handler->getNetNameOrFormedWithId(id) << ", expr = " << _handler->getNetExpression(id) << endl;
   }
   assert (ntk->getNetWidth(id1) == _p2cMap[id1.id].size());
...
V3BvBlastAig::bitBlast_CONST(const V3BvNtk* const ntk, const V3NetId& id) {
   ...
   const V3BitVecX* const value = ntk->getInputConstValue(id); assert (value);
