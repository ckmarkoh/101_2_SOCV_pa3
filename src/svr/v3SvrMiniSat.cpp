...
const bool
V3SvrMiniSat::simplify() { return _Solver->simplifyDB(); }
...
V3SvrMiniSat::add_DIV_Formula(const V3NetId& out, const uint32_t& depth) {
   ...
   const uint32_t index = getV3NetIndex(out); if(depth != _ntkData[index].size()) assert (depth == _ntkData[index].size());
   const uint32_t width = _ntk->getNetWidth(out); if(!width) assert (width);
   // Set SATVar
   // Build DIV I/O Relation
   const V3NetId in1 = _ntk->getInputNetId(out, 0); assert (validNetId(in1));
   const V3NetId in2 = _ntk->getInputNetId(out, 1); assert (validNetId(in2));
   const Var var1 = getVerifyData(in1, depth); if(!var1) assert (var1);
   const Var var2 = getVerifyData(in2, depth); if(!var2) assert (var2);
   ...
   if (BV_CONST == _ntk->getGateType(in2) && (sizeof(int) << 3) > _ntk->getNetWidth(in2)) {
      ...
      if (value->size() == value->firstX()) {
         ...
         for (int i = width; i > 0; --i) {
            ...
            else _Solver->addUnit(mkLit(var + i - 1, true));
         }
   ...
   if (BV_CONST == _ntk->getGateType(in2) && (sizeof(int) << 3) > _ntk->getNetWidth(in2)) {
      ...
      if (value->size() == value->firstX()) {
         ...
         for (int i = width; i > 0; --i) {	  
            ...
            else _Solver->addUnit(mkLit(var + width + i - shift - 1, true));
         }
         assert (getVerifyData(out, depth)); return;
      }
   }
...
V3SvrMiniSat::add_CONST_Formula(const V3NetId& out, const uint32_t& depth) {
   ...
   assert (value); assert (width == value->size());
   for (uint32_t i = 0; i < width; ++i)
      if ('X' != (*value)[i]) _Solver->addUnit(mkLit(var + i, '0' == (*value)[i]));
}
